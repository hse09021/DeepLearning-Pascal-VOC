...
class ResNetNew(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2)
        self.layer5 = self._make_detnet_layer(in_channels=2048)
        self.layer6 = self._make_detection_layer()  # 새로운 객체 탐지 레이어 추가
    ...
    def _make_detection_layer(self):
        # 객체 탐지를 위한 추가 레이어
        return nn.Sequential(
            ...
            nn.Sigmoid()  # 클래스 확률을 출력하기 위해 Sigmoid 사용
        )
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        x = self.layer5(x)
        x = self.layer6(x)  # 새로운 객체 탐지 레이어를 통과
...
def resnetNew(pretrained=False, **kwargs):
    model_ = ResNetNew(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model_.load_state_dict(model_zoo.load_url(
            'https://download.pytorch.org/models/resnet152-394f9c45.pth'))
    return model_
